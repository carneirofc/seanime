...
func GenerateTypescriptEndpointsFile(handlersJsonPath string, structsJsonPath string, outDir string, eventDir string) ([]string, error) {
	handlers, err := LoadHandlers(handlersJsonPath)
	if err != nil {
		return nil, err
	}
	structs, err := LoadPublicStructs(structsJsonPath)
	if err != nil {
		return nil, err
	}
	...
	osF, err := os.Create(filepath.Join(outDir, typescriptEndpointsFileName))
	if err != nil {
		return nil, fmt.Errorf("creating %s: %w", typescriptEndpointsFileName, err)
	}
	defer osF.Close()
	f := newErrWriter(osF)

	osTypef, err := os.Create(filepath.Join(outDir, typescriptEndpointTypesFileName))
	if err != nil {
		panic(err)
	}
	defer osTypef.Close()
	typeF := newErrWriter(osTypef)

	osHooksf, err := os.Create(filepath.Join(outDir, typescriptHooksFileName))
	if err != nil {
		panic(err)
	}
	defer osHooksf.Close()
	hooksF := newErrWriter(osHooksf)
...
`)
	...
	if err := generateEventFile(eventDir, endpointsMap); err != nil {
		return nil, err
	}

	for _, w := range []*errWriter{f, typeF, hooksF} {
		if err := w.Err(); err != nil {
			return nil, err
		}
	}

	return referenceGoStructs, nil
}

func generateHooksFile(f *errWriter, groupedHandlers map[string][]*RouteHandler, filenames []string) {
...
func generateEventFile(eventDir string, endpointsMap map[string]string) error {
...
func writeParamField(f *errWriter, handler *RouteHandler, param *RouteHandlerParam) {
...
func getEndpointKey(s string, groupName string) string {
	...
	for _, a := range endpointKeyAcronyms {
		result = strings.Replace(result, a.From, a.To, 1)
	}
	return strings.ReplaceAll(groupName, "_", "-") + "-" + result
}

func writeLine(file *errWriter, template string) {
