...
Seanime web is Next.js 15 app: primary UI for every Seanime client (browser, Denshi, Tauri). Doc captures "agents" - major runtimes and responsibility slices - inside subproject so contributors find right entry point fast.
...
| Build Targets | Environment-specific builds | `.env.*`, `next.config.{js|mjs}` | Next.js build pipeline | `npm run build[:desktop|:denshi]` |
...
`.env.web`, `.env.desktop`, `.env.denshi`, `.env.mobile` configure API endpoints, feature toggles, and analytics per target.
Scripts in `package.json` wrap Next CLI: `npm run dev` (browser), `npm run dev:desktop` and `npm run dev:denshi` (sidecar shells), `npm run build`, `npm run build:desktop`, and `npm run build:denshi` (static exports consumed by Go embedding or desktop apps).
Output directories (`out`, `out-desktop`, `out-denshi`) must be copied to `web/`, `web-desktop/`, or `seanime-denshi/web-denshi/` before packaging.
