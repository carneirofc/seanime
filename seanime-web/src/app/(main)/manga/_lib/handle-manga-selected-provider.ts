...
export function useSelectedMangaFilters(
    ...
    return {
        selectedFilters: storedFilters[key] || { scanlators: [], language: "", sourceProvider: "" },
        setSelectedScanlator: ({ mId, scanlators }: { mId: Nullish<string | number>, scanlators: string[] }) => {
            ...
            setStoredFilters(draft => {
                draft[key] ??= { scanlators: [], language: "", sourceProvider: "" }
        ...
        // Fork addition: sourceProvider is client-side only, so it is not sent to savePreference.
