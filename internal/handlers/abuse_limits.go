...
func shouldRateLimitMutation(req *http.Request) bool {
	...
	// The MCP server is read-only and uses POST as its JSON-RPC transport, so
	// its requests must not be counted against the mutation budget.
	if strings.HasPrefix(req.URL.Path, "/api/v1/mcp") {
		return false
	}
